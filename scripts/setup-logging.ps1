...
Write-Host "  weather-logs-*        pod stdout (Fluent Bit DaemonSet) - source of truth"
Write-Host "  weather-logs-cd-*     Jenkins pipeline console (optional, Fluent Bit on Jenkins)"
Write-Host "  (test JVM logs are kept as local Jenkins artifacts, not shipped to ES)"
Write-Host ""
Write-Host "For Jenkins CD console shipping, run:"
