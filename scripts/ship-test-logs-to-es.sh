#!/usr/bin/env bash
# Pushes newline-delimited JSON test logs into Elasticsearch via _bulk.
#
# Args: $1 = log file path, $2 = suite tag (mat|regression)
# Env:  ES_URL, BUILD_NUMBER
...
with open(sys.argv[1], encoding="utf-8") as fh:
    body = json.load(fh)
if body.get("errors"):
    failed = [i for i in body.get("items", []) if i.get("index", {}).get("error")]
    print(f"Bulk errors ({len(failed)} failed).", file=sys.stderr)
    if failed:
